...
public class DNSLookupService {
    ...
    private static Set<ResourceRecord> getResults(DNSNode node, int indirectionLevel) {
        ...
        Set<ResourceRecord> answer =  cache.getCachedResults(node);
        if (!answer.isEmpty()) {
            return answer;
        }
        ...
        // check again after recursively querying server
        ...
        if (answer.isEmpty()) {
            ...
            answer = cache.getCachedResults(new DNSNode(node.getHostName(), RecordType.CNAME));

            // if it is an alias we need to start from beginning with "true" name
            if(!answer.isEmpty()) {
                // query again to the next level
                ResourceRecord result = (ResourceRecord) answer.toArray()[0];
                return getResults(new DNSNode(result.getTextResult(), node.getType()), indirectionLevel + 1);
            }else{
                // If we don't have a CNAME neither, it's an exception.
                return Collections.emptySet();
            }
        }
    ...
    private static void retrieveResultsFromServer(DNSNode node, InetAddress server) {
        ...
        // System.out.println("Retrieving results from "+server+"\n");
    ...
    private static void queryNextLevel(DNSNode node, Set<ResourceRecord> nameservers) {
        ...
        // if we acquired the answer we were looking for
        ...
        while (it.hasNext()){
            if (it.next().getType() != RecordType.NS){
                return;
            }
        }

        // choose the first name server
        ResourceRecord nameserver = (ResourceRecord) nameservers.toArray()[0];

        // check if we have the name server's IP address in cache
        Set<ResourceRecord> recordSet = cache.getCachedResults(new DNSNode(nameserver.getTextResult(), RecordType.A));

        if(recordSet.isEmpty()){
            // System.out.println("We don't have name server's IP, finding now...\n");

            // we need to find the server's IP address
            recordSet = getResults(new DNSNode(nameserver.getTextResult(), RecordType.A), 0);

            // System.out.println("Querying for name server's IP\n");
        }



        // we have the IP addresses of the name servers so we can query the name server
        ResourceRecord record = (ResourceRecord) recordSet.toArray()[0];

        // System.out.format("Name server's IP address is "+ record.getInetResult() +"\n");
